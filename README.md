...
- **拖拽接线画布**：设备面板拖入生成节点，节点间连线即路由（严格单向：输出端子 → 输入端子）；点连线调混音音量/**实测路径延迟**（向源节点注入扫频脉冲 + 相关检测，含边缓冲/重采样/DSP 全程）/断开，点节点调采集开关与输出音量；**DSP 节点（开关/增益/延迟/EQ 等）的旁路开关直接放在节点上**，增益/延迟还有内嵌滑杆，详细参数走节点悬浮面板；布局自动持久化
...
- [x] **P2 · 功能节点（DSP）**：已有 增益 / 开关 / 延迟 / 三段 EQ / 参量 EQ / 图形 EQ / 高通 / 低通 /
      带通 / 限制器；旁路开关直接在节点上，增益/延迟带内嵌滑杆（压缩器「强度」未做）
- [x] **P2 · 节点音量指示条**（绿→黄→红分段阈值 + 峰值保持）与 **FFT 频谱展示（可开关）**（已完成）
- [ ] **P2 · 界面**：设备面板与画布等高、虚拟声卡线路两行布局、缩放时节点位置保持、
      节点文字换行、连线箭头不出圈、拉线虚线跟随（接线图自动排布已完成）
- [x] **P3 · 单实例**（重复启动唤醒已有窗口）；关闭窗口即销毁 WebView、重开时重建（界面不常驻内存）
- [ ] **P3 · 产品化剩余**：开机自启做到「无窗口、自动恢复线缆附加」；便携版（配置/日志跟随 exe）
