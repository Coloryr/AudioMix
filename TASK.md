...
1. **开机自启（无窗口）**：现在只写了当前用户注册表 Run 键 + `--headless`，要把它做扎实：
   ...
   - 自启后**自动恢复上次的线缆附加**：`usbip attach` 首次需要 UAC，而开机自启的进程是普通权限 ——
     可选做法：用 usbip-win2 自带的"登录/开机自动附加"机制；或把应用注册成登录时运行的提权计划任务；
     或让服务器先起来、由守护线程发现主机已附加后再对齐流（`Engine::refresh_devices` 已具备该能力）。
